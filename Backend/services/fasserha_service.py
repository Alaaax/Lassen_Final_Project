"""
خدمة ميزة "فسرها لي":
- تحميل 3 نماذج تصنيف (البحر، العصر، الموضوع) من مسار محلي أو Hugging Face.
- تحليل البيت/الأبيات عبر GPT بناءً على نتائج التصنيف.
- تجهيز استجابة JSON جاهزة للواجهة.
"""
...
def _load_hf_or_local_assets(path_or_repo: str):
    """
    يدعم:
    - مسار محلي: /opt/models/...
    - HF repo id: username/repo-name
    """
    token = _hf_token()

    common_kwargs = {
        "low_cpu_mem_usage": False,  # يمنع meta-init المسبب للمشكلة
    }
...
def _resolve_labels_file(topic_model_path_or_repo: str, labels_ref: str) -> str:
    """
    labels_ref يمكن يكون:
    - مسار ملف كامل محلي
    - اسم ملف داخل مودل الموضوع (مثل: label_info.pkl)
    """
    labels_ref = labels_ref.strip()

    # 1) إذا labels_ref مسار ملف محلي كامل
    if Path(labels_ref).is_file():
        return labels_ref

    # 2) إذا مودل الموضوع محلي + labels_ref اسم ملف
    if _is_local_path(topic_model_path_or_repo):
        local_candidate = Path(topic_model_path_or_repo) / labels_ref
        if local_candidate.is_file():
            return str(local_candidate)

    # 3) تحميل من Hugging Face repo
    token = _hf_token()
    downloaded_path = hf_hub_download(
        repo_id=topic_model_path_or_repo,
        filename=labels_ref,
        repo_type="model",
        token=token,
    )
...
def _get_meter_assets():
    model_path = _get_env_required("FASSERHA_METER_MODEL_PATH")
    return _load_hf_or_local_assets(model_path)


@lru_cache(maxsize=1)
def _get_era_assets():
    model_path = _get_env_required("FASSERHA_ERA_MODEL_PATH")
    return _load_hf_or_local_assets(model_path)
...
def predict_topic(poem_text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "top3": [
            {
                "label": _topic_to_ar(id2label_topic[int(i)]),
...
def _build_user_prompt(poem_text: str, meter_name: str, era_name: str, topic_name: str) -> str:
...
def _generate_explanation(poem_text: str, meter_name: str, era_name: str, topic_name: str) -> str:
    user_prompt = _build_user_prompt(poem_text, meter_name, era_name, topic_name)
...
def fasserha_li(poem_text: str) -> dict[str, Any]:
    ...
    explanation = _generate_explanation(poem_text, meter_name, era_name, topic_name)
...
def fasserha_api_response(poem_text: str) -> dict[str, Any]:
