"""
خدمة ميزة "ساعدني أكتب" - توليد الأبيات.
"""
...
METERS = [
    ...
    "المتقارب", "المتدارك", "المديد", "المضارع",
    "المقتضب", "المجتث",
]
...
def validate_idea(idea_text: str) -> tuple[bool, str]:
    ...
    تحقق ثنائي الطبقات:
    1. قواعد محلية سريعة
    2. تحقق دلالي عبر GPT
    """
    idea_clean = (idea_text or "").strip()

    # ── الطبقة 1: قواعد محلية ─────────────────────────────────
    if not idea_clean:
    ...
    if len(arabic_chars) < 2:
        return False, ERROR_MESSAGES["not_arabic"]

    if len(arabic_chars) / max(1, len(idea_clean)) < 0.3:
        return False, ERROR_MESSAGES["invalid_text"]

    # ── الطبقة 2: تحقق دلالي بـ GPT ──────────────────────────
...
def help_me_write_generate_api_response(
    idea_text:  str,
    meter_num:  int = 1,
    num_verses: int = 4,
) -> dict[str, Any]:
    """الاستجابة الجاهزة للـ API."""

    if not 1 <= meter_num <= len(METERS):
        return {"success": False, "message": f"meter_num يجب أن يكون بين 1 و{len(METERS)}"}
...
def generate_poetry_response(idea: str, meter_num: int = 1, num_verses: int = 4) -> dict[str, Any]:
    """اسم توافقي للاستخدام في main.py"""
